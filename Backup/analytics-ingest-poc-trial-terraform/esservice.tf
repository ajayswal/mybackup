resource "ibm_resource_instance" "event-stream" {
...
resource "ibm_resource_key" "EvenSt-resourceKey" {
  ...
  role                 = "${var.role}"
  resource_instance_id = "${ibm_resource_instance.event-stream.id}"
}

resource "ibm_container_bind_service" "EvenSt-iks-bind" {
  cluster_name_id     = "${ibm_container_cluster.iks-cluster.id}"
  space_guid          = "${data.ibm_space.space.id}"
  service_instance_id = "${ibm_resource_instance.event-stream.name}"
