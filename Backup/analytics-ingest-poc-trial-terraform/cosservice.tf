...
resource "ibm_resource_key" "cos-resource-key" {
  ...
  role                 = "${var.role}"
  resource_instance_id = "${ibm_resource_instance.cos-resource-instance.id}"
...
output "resource_id" {
  value = "${ibm_resource_instance.cos-resource-instance.id}"
}

resource "ibm_container_bind_service" "objectstorage-iks-bind" {
  cluster_name_id     = "${ibm_container_cluster.iks-cluster.id}"
  space_guid          = "${data.ibm_space.space.id}"
  service_instance_id = "${ibm_resource_instance.cos-resource-instance.name}"
